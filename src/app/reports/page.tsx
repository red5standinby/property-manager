      ...
      <div className="p-6 lg:p-8">
        <div className="mb-8 flex flex-wrap items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-xs text-muted-foreground/70">Generate, schedule, and download portfolio reports</p>
          </div>
          <button className="inline-flex items-center gap-1.5 rounded-xl border border-border bg-muted/60 px-3.5 py-2 text-xs font-semibold text-foreground/90 transition-all hover:bg-muted">
        ...
        <div className="mb-10 grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-3">
          {reportTypes.map((r) => (
            <div
              key={r.title}
              className="group relative overflow-hidden rounded-2xl border border-border bg-card/40 p-5 transition-all duration-200 hover:border-foreground/20 hover:shadow-[0_0_0_1px_rgba(255,255,255,0.04),0_8px_32px_rgba(0,0,0,0.4)] cursor-pointer"
            >
              ...
              <div className="relative">
                <div className="mb-4 flex items-start justify-between">
                  ...
                  <ArrowRight className="h-3.5 w-3.5 -translate-x-1 text-muted-foreground/30 opacity-0 transition-all group-hover:translate-x-0 group-hover:opacity-100" />
                </div>
                <p className="mb-1 text-sm font-medium text-foreground/90">{r.title}</p>
                <p className="mb-3 text-xs leading-relaxed text-muted-foreground/70">{r.description}</p>
                <div className="flex flex-wrap gap-1.5">
                  {r.tags.map((t) => (
                    <span key={t} className="rounded-md bg-muted/80 px-2 py-0.5 text-[10px] font-medium text-muted-foreground/70">
                      {t}
                    </span>
                  ))}
                </div>
        ...
        <div className="rounded-2xl border border-border bg-card/40 overflow-hidden">
          <div className="flex items-center justify-between border-b border-border px-5 py-4">
            <div>
              <p className="text-sm font-medium text-foreground/90">Recent Reports</p>
              <p className="text-[11px] text-muted-foreground/70">Previously generated exports</p>
            </div>
          </div>

          <div className="divide-y divide-border">
            {recentReports.map((r, i) => (
              <div key={i} className="flex items-center gap-4 px-5 py-3.5 transition-colors hover:bg-muted/20">
                <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-muted">
                  <ChartBar weight="duotone" className="h-4 w-4 text-muted-foreground/70" />
                </div>
                <div className="min-w-0 flex-1">
                  <p className="truncate text-xs font-medium text-foreground/90">{r.name}</p>
                  <p className="text-[10px] text-muted-foreground/70">{r.date} · {r.size}</p>
                </div>
