  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-1 text-xs text-zinc-500">Manage your account details and preferences.</p>
      </div>

      {/* Avatar */}
      <div className="flex items-center gap-5 rounded-2xl border border-zinc-800/60 bg-zinc-900/40 p-5">
        ...
        <div>
          <p className="text-sm font-medium text-zinc-200">Jaime Luna</p>
          <p className="text-xs text-zinc-500">theluna@gmail.com</p>
      ...
      {/* Fields */}
      <div className="rounded-2xl border border-zinc-800/60 bg-zinc-900/40 divide-y divide-zinc-800/40 overflow-hidden">
        {[
          ...
          <div key={f.label} className="flex items-center justify-between px-5 py-4">
            <div className="min-w-0">
              <p className="text-[11px] font-medium text-zinc-500">{f.label}</p>
              <p className="mt-0.5 text-xs text-zinc-200">{f.value}</p>
            </div>
            <button className="shrink-0 text-[11px] font-medium text-blue-400 hover:text-blue-300">
              Edit
            </button>
          </div>
        ))}
      ...
      <div className="flex justify-end">
        <button className="inline-flex items-center gap-1.5 rounded-xl bg-blue-600 px-4 py-2 text-xs font-semibold text-white shadow-[0_0_12px_rgba(59,130,246,0.3)] transition-all hover:bg-blue-500">
          Save Changes
        </button>
      </div>
    </div>
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-1 text-xs text-zinc-500">Choose which events trigger emails or push notifications.</p>
      </div>
      {groups.map((g) => (
        <div key={g.label} className="rounded-2xl border border-zinc-800/60 bg-zinc-900/40 overflow-hidden">
          <div className="border-b border-zinc-800/60 px-5 py-3">
            <p className="text-xs font-semibold text-zinc-300">{g.label}</p>
          </div>
                ...
                <div className="flex items-center gap-4">
                  {[{ label: 'Email', val: item.email }, { label: 'Push', val: item.push }].map((t) => (
                    <label key={t.label} className="flex cursor-pointer items-center gap-1.5">
                      ...
                      <div className={`h-4 w-7 rounded-full transition-colors ${t.val ? 'bg-blue-600' : 'bg-zinc-700'}`}>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col items-center justify-center rounded-2xl border border-dashed border-zinc-800 bg-zinc-900/20 py-16 text-center">
        <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-zinc-800/60 mb-4">
          <CheckCircle weight="duotone" className="h-6 w-6 text-zinc-500" />
        </div>
        <p className="text-sm font-medium text-zinc-400">{title} settings</p>
        <p className="mt-1 text-xs text-zinc-600">This section is coming soon.</p>
      </div>
    </div>
  )
}
    ...
    <AppShell>
      <div className="p-6 lg:p-8">
        <div className="mb-8">
          ...
          <p className="mt-1 text-xs text-zinc-500">Manage your account, notifications, and organization.</p>
        </div>
              ...
              return (
                ...
                >
                  <tab.icon
                    ...
                    className={`h-4 w-4 shrink-0 ${active ? 'text-blue-400' : 'text-zinc-600'}`}
                  />
                  <span className="flex-1 text-left">{tab.label}</span>
                  {active && <CaretRight weight="bold" className="h-3 w-3 text-blue-400/60" />}
                </button>
              )
            })}
          </nav>
