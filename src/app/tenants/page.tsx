...
const statusStyles: Record<string, { badge: string; dot: string }> = {
  current:  { badge: 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/20', dot: 'bg-emerald-500' },
  expiring: { badge: 'bg-amber-500/10 text-amber-400 border border-amber-500/20',      dot: 'bg-amber-500'   },
  late:     { badge: 'bg-red-500/10 text-red-400 border border-red-500/20',            dot: 'bg-red-500'     },
}
    ...
    <AppShell>
      <div className="p-6 lg:p-8">
        ...
        <div className="rounded-2xl border border-zinc-800/60 bg-zinc-900/40 overflow-hidden">
          <div className="grid grid-cols-[auto_1fr_auto_auto_auto] items-center gap-4 border-b border-zinc-800/60 px-5 py-3">
            <span className="text-[10px] font-semibold uppercase tracking-widest text-zinc-600">Tenant</span>
            <span className="text-[10px] font-semibold uppercase tracking-widest text-zinc-600">Unit</span>
            <span className="hidden text-[10px] font-semibold uppercase tracking-widest text-zinc-600 sm:block">Contact</span>
            <span className="hidden text-[10px] font-semibold uppercase tracking-widest text-zinc-600 md:block">Lease</span>
          ...
          <div className="divide-y divide-zinc-800/40">
            {tenants.map((tenant, i) => {
              ...
              return (
                <div
                  key={tenant.id}
                  className="grid grid-cols-[auto_1fr_auto_auto_auto] items-center gap-4 px-5 py-3.5 transition-colors hover:bg-zinc-800/20 cursor-pointer"
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-[10px] text-zinc-500">Since {tenant.since}</p>
                    </div>
                  </div>
                  ...
                  <div className="hidden md:block">
                    <p className="text-[11px] text-zinc-400">{tenant.lease}</p>
                  </div>
